...
public class FastSoSoService {
	public static String httpUrl = "http://www.fastsoso.cn/search";
	...
	public static String[] request(String num ,int page) {
		...
		try {
            ...
            for(Element link : linksHref){
                ...
                if(link.attr("rel").equals("noreferrer")&&link.select("span").size()>0){//java:file-6881557.html
                ...
                }else if(link.attr("rel").equals("noreferrer")&&link.select("span").size()==0){//微风吹过的黎明:user-2452036771-1.html
                	String linkText = link.html();
                	if(str==null){
            			str = linkText+"@@"+topUrl+linkHref;
            		}else{
            			str = str +"," + linkText+"@@"+linkHref;
            		}
                }
            }
		} catch (Exception e) {
