...
function dataInit(){
	...
	$('#addMain').bootstrapValidator({
		...
		if(mainVedioId==""||mainVedioId==null){
			...
			var params = {
					optionType:optionType,//主文件类型
					optionId:fileId.toString(","),//附件id
					optionVedioId:mainVedioId,//主文件id
			...
			AjaxUtil.request({url:path+"/post/add/insertTsyUserCustomOptions",params:params,type:'json',callback:function(json){
...
function upLoadFile(){
	...
	$('#vedio').fileupload({
        dataType: 'json',
        url: path + '/post/TsyOptions/uploadVedio',
        done: function (e, data) {
        	$(".items-vedio .item").last().find(".delete").show();
        	$("#weixin_vedio").show();
        	$("#weixin_vedio span").html("重选");
        	$(".items-vedio .item").last().find(".ts").html("上传成功");
        	...
        	mainVedioId = data.result.bean.id;
        },
        progressall: function (e, data) {
        	var progress = parseInt(data.loaded / data.total * 100, 10);
        	$(".items-vedio .item").last().find(".ts").html("正在上传...");
            $(".items-vedio .item").last().find(".progress-bar-success").css('width',progress + '%');
            $(".items-vedio .item").last().find(".progress-bar-success").html(progress + '%');  
        },
        add: function (e, data) {
        	...
        	if(type=="mp4"||type=="rmvb"||type=="avi"||type=="3gp"){
        		...
        		for(var i = 0;i<data.files.length;i++){
        			$(".items-vedio").html($(".items-vedio").html()+uploadModelTwo().replace("{imgName}", data.files[i].name).replace("{imgSize}", bytesToSize(data.files[i].size)));
        		}
        		$("#weixin_vedio").hide();
        		TxVidoe();
        		data.submit();
