...
function dataInit(){
	id = $.req("id");
	AjaxUtil.request({url:path+"/post/GraphicMessage/updateGraphicMessageSelect",params:{id:id},type:'json',callback:function(json){
			//设置第一个图文内容
			var nowId = "topOne";
			$("#topOne").attr("rowId",json.bean.id);
			$("#topOne img").attr("src",path + "/" + json.bean.graphic_message_jieshao_pic);
			$("#topOne font").html(json.bean.graphic_message_title);
			graphicMessageFatherid = json.bean.id;
			graphicMessageJieshaoPic = json.bean.graphic_message_jieshao_pic;
			//设置剩余图文内容
			if(json.total>0){
				for(var i = 0;i<json.rows.length;i++){
					if(itemClick("addItem")){
						$(".bot-ul").find("li[id=addItemLi]").remove();
						if($(".bot-ul").find("li").length<=3){
							$(".bot-ul").html($(".bot-ul").html()+returnGraphicMessageItemData(json.rows[i].id,json.rows[i].graphicMessageTitle,path+"/"+json.rows[i].graphicMessageJieshaoPic)+returnGraphicMessageAddBtn());
						}else{
							$(".bot-ul").html($(".bot-ul").html()+returnGraphicMessageItemData(json.rows[i].id,json.rows[i].graphicMessageTitle,path+"/"+json.rows[i].graphicMessageJieshaoPic));
						}
					}
					if(id==json.rows[i].id){
						$("#NowConsoleId").attr("ConsoleId","li" + ($(".bot-ul").find("li").length-2));
						nowId = "li" + ($(".bot-ul").find("li").length-2);
					}
				}
			}
		}
...
function returnGraphicMessageItemData(rowId,title,pic){
	var id = "li" + $(".bot-ul").find("li").length;
	return '<li><a id="'+id+'" href="javascript:;" class="Grapnic-Message-item" rowId="'+rowId+'"><p>'+title+'</p><img src="'+pic+'" /></a></li>';
}
